...
final class UriTemplate
{
     ...
     * @link https://tools.ietf.org/html/rfc6570#section-2.2
     */
    const RESERVED_OPERATOR = '=,!@|';
    /**
     * Processing behavior according to the expression type operator.
     *
     * @link https://tools.ietf.org/html/rfc6570#appendix-A
     */
    const OPERATOR_HASH_LOOKUP = ['' => ['prefix' => '', 'joiner' => ',', 'query' => false], '+' => ['prefix' => '', 'joiner' => ',', 'query' => false], '#' => ['prefix' => '#', 'joiner' => ',', 'query' => false], '.' => ['prefix' => '.', 'joiner' => '.', 'query' => false], '/' => ['prefix' => '/', 'joiner' => '/', 'query' => false], ';' => ['prefix' => ';', 'joiner' => ';', 'query' => true], '?' => ['prefix' => '?', 'joiner' => '&', 'query' => true], '&' => ['prefix' => '&', 'joiner' => '&', 'query' => true]];
    /**
     * @var string
     */
    private $template;
    ...
    private function parseExpressions()
    {
        ...
        foreach ($expressions as $expression) {
            ...
            list($parsedVariables, $foundVariables) = $this->parseVariableSpecification($expression, $foundVariables);
    ...
    public function expand(array $variables = []) : UriInterface
    {
        ...
        if ([] === $this->variables) {
            $this->cache['noVariables'] = $this->cache['noVariables'] ?? Uri::createFromString(preg_replace(self::REGEXP_EXPRESSION, '', $this->template));
    ...
    private function expandVariable(array $variable, string $operator, string $joiner, bool $useQuery) : string
    {
        ...
        if (is_array($expanded)) {
            list($expanded, $actualQuery) = $expanded;
        }
        if (!$actualQuery) {
            return $expanded;
        }
