...
final class Uri implements UriInterface
{
     ...
     */
    const HOST_ADDRESS_BLOCK = "þ€";
    /**
     * Regular expression pattern to for file URI.
     */
    const REGEXP_FILE_PATH = ',^(?<delim>/)?(?<root>[a-zA-Z][:|\\|])(?<rest>.*)?,';
    /**
     ...
     * @link https://tools.ietf.org/html/rfc2397
     */
    const REGEXP_BINARY = ',(;|^)base64$,';
    /**
     * Windows file path string regular expression pattern.
     */
    const REGEXP_WINDOW_PATH = ',^(?<root>[a-zA-Z][:|\\|]),';
    /**
     * Supported schemes and corresponding default port.
     *
     * @var array
     */
    const SCHEME_DEFAULT_PORT = ['data' => null, 'file' => null, 'ftp' => 21, 'gopher' => 70, 'http' => 80, 'https' => 443, 'ws' => 80, 'wss' => 443];
    /**
     * URI validation methods per scheme.
     *
     * @var array
     */
    const SCHEME_VALIDATION_METHOD = ['data' => 'isUriWithSchemeAndPathOnly', 'file' => 'isUriWithSchemeHostAndPathOnly', 'ftp' => 'isNonEmptyHostUriWithoutFragmentAndQuery', 'gopher' => 'isNonEmptyHostUriWithoutFragmentAndQuery', 'http' => 'isNonEmptyHostUri', 'https' => 'isNonEmptyHostUri', 'ws' => 'isNonEmptyHostUriWithoutFragment', 'wss' => 'isNonEmptyHostUriWithoutFragment'];
    /**
     ...
     */
    private function __construct(string $scheme = null, string $user = null, string $pass = null, string $host = null, int $port = null, string $path, string $query = null, string $fragment = null)
    {
     ...
     */
    private function formatScheme(string $scheme = null)
    {
     ...
     */
    private function formatUserInfo(string $user = null, string $password = null)
    {
        if (null === $user) {
            return $user;
        }
     ...
     * @param ?string $host
     */
    private function formatHost(string $host = null)
    {
        if (null === $host || '' === $host) {
            return $host;
        }
     ...
     * @throws SyntaxError
     */
    private function formatPort($port = null)
    {
        if (null === $port || '' === $port) {
            return null;
        }
    ...
    public static function createFromUri($uri) : self
    {
        if ($uri instanceof UriInterface) {
            ...
            $pass = null;
            if (null !== $user_info) {
                list($user, $pass) = explode(':', $user_info, 2) + [1 => null];
            }
     ...
     */
    private function formatQueryAndFragment(string $component = null)
    {
     ...
     * @param ?string $fragment
     */
    private function getUriString(string $scheme = null, string $authority = null, string $path, string $query = null, string $fragment = null) : string
    {
        if (null !== $scheme) {
            $scheme = $scheme . ':';
        }
     ...
     * {@inheritDoc}
     */
    public function withUserInfo($user = NULL, $password = null)
    {
        $user_info = null;
