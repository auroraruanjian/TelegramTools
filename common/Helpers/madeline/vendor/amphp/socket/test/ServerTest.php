...
class ServerTest extends TestCase
{
    ...
    {
        Loop::run(function () {
            $tlsContext = (new Socket\ServerTlsContext())->withCertificates(['amphp.org' => new Socket\Certificate(__DIR__ . '/tls/amphp.org.pem')]);
            $server = Server::listen('127.0.0.1:0', (new Socket\BindContext())->withTlsContext($tlsContext));
            asyncCall(function () use($server) {
    ...
    {
        Loop::run(function () {
            $tlsContext = (new Socket\ServerTlsContext())->withCertificates(['amphp.org' => new Socket\Certificate(__DIR__ . '/tls/amphp.org.pem'), 'www.amphp.org' => new Socket\Certificate(__DIR__ . '/tls/www.amphp.org.pem')]);
            $server = Server::listen('127.0.0.1:0', (new Socket\BindContext())->withTlsContext($tlsContext));
            asyncCall(function () use($server) {
                /** @var Socket\ResourceSocket $socket */
                while ($socket = (yield $server->accept())) {
            ...
            (yield $client->setupTls());
            (yield $client->write('Hello World'));
            $context = (new Socket\ConnectContext())->withTlsContext((new Socket\ClientTlsContext('www.amphp.org'))->withCaFile(__DIR__ . '/tls/www.amphp.org.crt'));
            /** @var Socket\EncryptableSocket $client */
            $client = (yield Socket\connect($server->getAddress(), $context));
            (yield $client->setupTls());
            (yield $client->write('Hello World'));
    ...
    public function testSniWorksWithMultipleCertificatesAndDifferentFilesForCertAndKey()
    {
        ...
        Loop::run(function () {
            $tlsContext = (new Socket\ServerTlsContext())->withCertificates(['amphp.org' => new Socket\Certificate(__DIR__ . '/tls/amphp.org.crt', __DIR__ . '/tls/amphp.org.key'), 'www.amphp.org' => new Socket\Certificate(__DIR__ . '/tls/www.amphp.org.crt', __DIR__ . '/tls/www.amphp.org.key')]);
            $server = Server::listen('127.0.0.1:0', (new Socket\BindContext())->withTlsContext($tlsContext));
            asyncCall(function () use($server) {
                /** @var Socket\ResourceSocket $socket */
                while ($socket = (yield $server->accept())) {
