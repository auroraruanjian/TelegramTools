 ...
 * @internal
 */
function setupTls($socket, array $options, CancellationToken $cancellationToken = null) : Promise
{
    ...
    if ($result === false) {
        return new Failure(new TlsException('TLS negotiation failed: ' . (\error_get_last()['message'] ?? 'Unknown error')));
    }
    return call(static function () use($socket, $cancellationToken) {
        ...
        $watcher = Loop::onReadable($socket, static function (string $watcher, $socket, Deferred $deferred) use($cancellationToken, $id) {
            ...
            if ($result === true) {
                ...
                Loop::cancel($watcher);
                $cancellationToken->unsubscribe($id);
                $deferred->fail(new TlsException('TLS negotiation failed: ' . (\feof($socket) ? 'Connection reset by peer' : \error_get_last()['message'])));
            }
        }, $deferred);
