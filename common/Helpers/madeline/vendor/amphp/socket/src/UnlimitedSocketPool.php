...
{
    const ALLOWED_SCHEMES = ['tcp' => null, 'unix' => null];
    private $sockets = [];
    ...
    public function checkout(string $uri, ConnectContext $context = null, CancellationToken $token = null) : Promise
    {
        ...
        list($uri, $fragment) = $this->normalizeUri($uri);
        $cacheKey = $uri;
    ...
    /** @inheritdoc */
    public function clear(EncryptableSocket $socket)
    {
        $this->clearFromId(\spl_object_hash($socket));
    }
    /** @inheritdoc */
    public function checkin(EncryptableSocket $socket)
    {
    ...
    private function checkoutNewSocket(string $uri, string $cacheKey, ConnectContext $connectContext, CancellationToken $token = null) : Promise
    {
     ...
     */
    private function clearFromId(string $objectId)
    {
        if (!isset($this->objectIdCacheKeyMap[$objectId])) {
            throw new \Error(\sprintf('Unknown socket: %d', $objectId));
        }
