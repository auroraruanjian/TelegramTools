...
class WebSocketTest extends AsyncTestCase
{
    ...
    {
        list($server, $port) = (yield $this->createServer(new Websocket(new class extends Helper\WebsocketAdapter
        {
    ...
    {
        $options = Options::createClientDefault()->withBytesPerSecondLimit(\PHP_INT_MAX)->withFramesPerSecondLimit(\PHP_INT_MAX)->withMessageSizeLimit(1024 * 1024 * 10)->withoutCompression();
        list($server, $port) = (yield $this->createServer(new Websocket(new class extends Helper\WebsocketAdapter
        {
            public function handleClient(Client $client, Request $request, Response $response) : Promise
            {
                $payload = \str_repeat('.', 1024 * 1024 * 10);
                // 10 MiB
    ...
    public function testTooLongMessage() : \Generator
    {
        ...
        list($server, $port) = (yield $this->createServer(new Websocket(new class extends Helper\WebsocketAdapter
        {
            public function handleClient(Client $client, Request $request, Response $response) : Promise
            {
                $payload = \str_repeat('.', 1024 * 1024 * 10 + 1);
                // 10 MiB + 1 byte
                return $client->sendBinary($payload);
            }
        })));
