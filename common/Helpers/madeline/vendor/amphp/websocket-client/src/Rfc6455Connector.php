...
class Rfc6455Connector implements Connector
{
    ...
    public function connect(Handshake $handshake, CancellationToken $cancellationToken = null) : Promise
    {
        return call(function () use($handshake, $cancellationToken) {
            ...
            $request->setUpgradeHandler(function (EncryptableSocket $socket, Request $request, Response $response) use($deferred, $key, $options) {
            ...
            if ($response->getStatus() !== Http\Status::SWITCHING_PROTOCOLS) {
                throw new ConnectionException(\sprintf('A %s (%d) response was not received; instead received response status: %s (%d)', Http\Status::getReason(Http\Status::SWITCHING_PROTOCOLS), Http\Status::SWITCHING_PROTOCOLS, $response->getReason(), $response->getStatus()), $response);
            }
    ...
    private function generateRequest(Handshake $handshake, string $key) : Request
    {
        ...
        if (!$request->hasHeader('origin')) {
            $origin = $uri->withUserInfo('')->withPath('')->withQuery('');
        ...
        if (!empty($extensions)) {
            $pairs = [];
            foreach ($extensions as list($name, $value)) {
                if ($value === '') {
                    $pairs[] = $name;
                    continue;
                }
     ...
     */
    protected final function createCompressionContext(Response $response)
    {
