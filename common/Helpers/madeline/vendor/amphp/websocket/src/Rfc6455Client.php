...
final class Rfc6455Client implements Client
{
     ...
     */
    public function __construct(Socket $socket, Options $options, bool $masked, CompressionContext $compression = null)
    {
        $this->socket = $socket;
        ...
        if (self::$watcher === null) {
            ...
            self::$watcher = Loop::repeat(1000, static function () {
                self::$now = \time();
    ...
    private function read() : \Generator
    {
        ...
        try {
            while (($chunk = (yield $this->socket->read())) !== null) {
                ...
                $parser->send($chunk);
                $chunk = '';
                // Free memory from last chunk read.
                if ((self::$framesReadInLastSecond[$this->metadata->id] ?? 0) >= $maxFramesPerSecond || self::$bytesReadInLastSecond[$this->metadata->id] >= $maxBytesPerSecond) {
                    Loop::reference(self::$watcher);
                    // Reference watcher to keep loop running until rate limit released.
    ...
    private function onData(int $opcode, string $data, bool $terminated)
    {
        ...
        if (!$this->currentMessageEmitter) {
            if ($opcode === Opcode::CONT) {
                $this->onError(Code::PROTOCOL_ERROR, 'Illegal CONTINUATION opcode; initial message payload frame must be TEXT or BINARY');
                return;
            }
            ...
            $this->onError(Code::PROTOCOL_ERROR, 'Illegal data type opcode after unfinished previous data type frame; opcode MUST be CONTINUATION');
            return;
        }
    ...
    private function onControlFrame(int $opcode, string $data)
    {
        ...
        switch ($opcode) {
            case Opcode::CLOSE:
                ...
                if ($length === 0) {
                    ...
                    if ($code < 1000 || $code >= 1004 && $code <= 1006 || $code >= 1014 && $code <= 1015 || $code >= 1016 && $code <= 1999 || $code >= 2000 && $code <= 2999 || $code >= 5000) {
                        $code = Code::PROTOCOL_ERROR;
    ...
    private function onError(int $code, string $reason)
    {
        $this->close($code, $reason);
    }
    ...
    private function sendData(string $data, int $opcode) : \Generator
    {
        ...
        $rsv = 0;
        $compress = false;
        if ($this->compressionContext && $opcode === Opcode::TEXT && \strlen($data) > $this->compressionContext->getCompressionThreshold()) {
        ...
        try {
            ...
            if (\strlen($data) > $this->options->getFrameSplitThreshold()) {
                ...
                while ($data !== '') {
                    ...
                    $opcode = Opcode::CONT;
                    $rsv = 0;
                    // RSV must be 0 in continuation frames.
                }
            } else {
                if ($compress) {
    ...
    private function parser() : \Generator
    {
        ...
        while (true) {
            $payload = '';
            // Free memory from last frame payload.
            while ($bufferSize < 2) {
            ...
            if ($isControlFrame || $opcode === Opcode::CONT) {
                // Control and continuation frames
                if ($rsv !== 0) {
            ...
            if ($frameLength === 0x7e) {
                ...
                if (\PHP_INT_MAX === 0x7fffffff) {
                    if ($lengthLong32Pair[1] !== 0 || $lengthLong32Pair[2] < 0) {
                        $this->onError(Code::MESSAGE_TOO_LARGE, 'Received payload exceeds maximum allowable size');
                        return;
                    }
                    ...
                    if ($frameLength < 0) {
                        $this->onError(Code::PROTOCOL_ERROR, 'Most significant bit of 64-bit length field set');
                        return;
                    }
                }
            }
            if ($frameLength > 0 && $isMasked === $this->masked) {
                $this->onError(Code::PROTOCOL_ERROR, 'Payload mask error');
                return;
            }
            if ($isControlFrame) {
                if (!$final) {
                    $this->onError(Code::PROTOCOL_ERROR, 'Illegal control frame fragmentation');
                    return;
                }
                if ($frameLength > 125) {
                    $this->onError(Code::PROTOCOL_ERROR, 'Control frame payload must be of maximum 125 bytes or less');
                    return;
                }
            }
            ...
            if ($doUtf8Validation) {
                if ($final) {
                    ...
                    for ($i = 0; !($valid = \preg_match('//u', $payload)); $i++) {
                        ...
                        if ($i === 3) {
                            // Remove a maximum of three bytes
                            break;
                        }
                    }
                }
                if (!$valid) {
                    $this->onError(Code::INCONSISTENT_FRAME_DATA_TYPE, 'Invalid TEXT data; UTF-8 required');
                    return;
                }
            }
            if ($final) {
