...
final class Rfc1035StubResolver implements Resolver
{
    ...
    public function query(string $name, int $type) : Promise
    {
        ...
        $promise = call(function () use($name, $type) {
            ...
            throw new TimeoutException(\sprintf("No response for '%s' (%s) from any nameserver after %d attempts, tried %s", $name, Record::getName($type), $attempts, \implode(", ", $attemptDescription)));
        });
