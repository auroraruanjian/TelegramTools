...
abstract class ConnectionPool implements Pool
{
     ...
     * @return Connector
     */
    protected abstract function createDefaultConnector() : Connector;
    /**
     ...
     * @return ResultSet
     */
    protected abstract function createResultSet(ResultSet $resultSet, callable $release) : ResultSet;
    /**
     ...
     * @return Statement
     */
    protected abstract function createStatement(Statement $statement, callable $release) : Statement;
    /**
     * @param Pool      $pool
     * @param Statement $statement
     ...
     */
    protected abstract function createStatementPool(Pool $pool, Statement $statement, callable $prepare) : StatementPool;
    /**
     ...
     * @return Transaction
     */
    protected abstract function createTransaction(Transaction $transaction, callable $release) : Transaction;
    /**
     ...
     */
    public function __construct(ConnectionConfig $config, int $maxConnections = self::DEFAULT_MAX_CONNECTIONS, int $idleTimeout = self::DEFAULT_IDLE_TIMEOUT, Connector $connector = null)
    {
    ...
    protected function pop() : \Generator
    {
        ...
        while ($this->promise !== null) {
            (yield $this->promise);
            // Prevent simultaneous connection creation or waiting.
        }
        do {
                ...
                if ($this->connections->count() < $this->getConnectionLimit()) {
                    ...
                    try {
                        ...
                        if (!$connection instanceof Link) {
                            throw new \Error(\sprintf("%s::connect() must resolve to an instance of %s", \get_class($this->connector), Link::class));
                        }
                    } finally {
                        $this->promise = null;
                    }
    ...
    public function prepare(string $sql) : Promise
    {
        return call(function () use($sql) {
            ...
            return $this->createStatementPool($this, $statement, coroutine($this->callableFromInstanceMethod("prepareStatement")));
        });
    }
    /**
