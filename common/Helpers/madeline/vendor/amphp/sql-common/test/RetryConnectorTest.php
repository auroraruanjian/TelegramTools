...
class RetryConnectorTest extends AsyncTestCase
{
    public function testSuccessfulConnect() : \Generator
    {
        ...
        $connector->expects($this->once())->method('connect')->willReturn(new Success($this->createMock(Link::class)));
        $retry = new RetryConnector($connector);
        $config = $this->getMockBuilder(ConnectionConfig::class)->setConstructorArgs(['localhost', 5432])->getMockForAbstractClass();
    ...
    {
        $connector = $this->createMock(Connector::class);
        $connector->expects($this->exactly(2))->method('connect')->willReturnOnConsecutiveCalls(new Failure(new ConnectionException()), new Success($this->createMock(Link::class)));
        $retry = new RetryConnector($connector);
        $config = $this->getMockBuilder(ConnectionConfig::class)->setConstructorArgs(['localhost', 5432])->getMockForAbstractClass();
        $connection = (yield $retry->connect($config));
        $this->assertInstanceOf(Link::class, $connection);
    ...
    public function testFailingConnect() : \Generator
    {
        $tries = 3;
        $connector = $this->createMock(Connector::class);
        $connector->expects($this->exactly($tries))->method('connect')->willReturn(new Failure(new ConnectionException()));
