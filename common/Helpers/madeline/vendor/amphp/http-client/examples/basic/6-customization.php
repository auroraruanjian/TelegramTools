...
Loop::run(static function () use($argv) {
    try {
        $client = (new HttpClientBuilder())->intercept(new LogHttpArchive(__DIR__ . '/log.har'))->intercept(new MatchOrigin(['https://amphp.org' => new SetRequestHeader('x-amphp', 'true')]))->followRedirects(0)->retry(3)->build();
        for ($i = 0; $i < 5; $i++) {
