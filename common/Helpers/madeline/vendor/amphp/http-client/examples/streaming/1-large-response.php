...
Loop::run(static function () {
    try {
        ...
        $request->setBodySizeLimit(128 * 1024 * 1024);
        // 128 MB
        ...
        // Output the results
        \printf("HTTP/%s %d %s\r\n%s\r\n\r\n", $response->getProtocolVersion(), $response->getStatus(), $response->getReason(), $response->getRequest()->getUri());
        ...
        while (null !== ($chunk = (yield $response->getBody()->read()))) {
            ...
            $bytes += \strlen($chunk);
            print "\r" . formatBytes($bytes) . '    ';
            // blanks to remove previous output
        }
        (yield $file->close());
        print \sprintf("\rDone in %.2f seconds with peak memory usage of %.2fMB.\n", (getCurrentTime() - $start) / 1000, (float) \memory_get_peak_usage(true) / 1024 / 1024);
