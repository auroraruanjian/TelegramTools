...
final class Http2ConnectionProcessor implements Http2Processor
{
    ...
    const MINIMUM_WINDOW = 512 * 1024;
    const WINDOW_INCREMENT = 1024 * 1024;
    ...
    public function initialize() : Promise
    {
        ...
        if ($this->socket->isClosed()) {
            return new Failure(new UnprocessedRequestException(new SocketException('The socket closed before the connection could be initialized')));
        }
    ...
    {
        $message = \sprintf("Received GOAWAY frame from %s with error code %d", $this->socket->getRemoteAddress(), $error);
    ...
    public function handleStreamWindowIncrement(int $streamId, int $windowSize)
    {
        ...
        if ($stream->clientWindow + $windowSize > 2147483647) {
            $this->handleStreamException(new Http2StreamException("Current window size plus new window exceeds maximum size", $streamId, Http2Parser::FLOW_CONTROL_ERROR));
            return;
        }
    ...
    public function handleConnectionWindowIncrement(int $windowSize)
    {
        if ($this->clientWindow + $windowSize > 2147483647) {
            $this->handleConnectionException(new Http2ConnectionException("Current window size plus new window exceeds maximum size", Http2Parser::FLOW_CONTROL_ERROR));
            return;
        }
    ...
    public function handleHeaders(int $streamId, array $pseudo, array $headers, bool $streamEnded)
    {
        ...
        if ($stream->trailers) {
            if ($stream->expectedLength && $stream->received !== $stream->expectedLength) {
                ...
                $this->handleStreamException(new Http2StreamException("Content length mismatch: " . \abs($diff) . ' bytes ' . ($diff > 0 ? ' missing' : 'too much'), $streamId, Http2Parser::PROTOCOL_ERROR));
                return;
            }
            if (!empty($pseudo)) {
                $this->handleStreamException(new Http2StreamException("Trailers must not contain pseudo headers", $streamId, Http2Parser::PROTOCOL_ERROR));
                return;
            }
            try {
                ...
                $this->handleStreamException(new Http2StreamException("Disallowed field names in trailer", $streamId, Http2Parser::PROTOCOL_ERROR, $exception));
                return;
            }
        ...
        if (!isset($pseudo[":status"])) {
            $this->handleConnectionException(new Http2ConnectionException("No status pseudo header in response", Http2Parser::PROTOCOL_ERROR));
            return;
        }
        if (!\preg_match("/^[1-5]\\d\\d\$/", $pseudo[":status"])) {
            $this->handleStreamException(new Http2StreamException("Invalid response status code: " . $pseudo[':status'], $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        if ($stream->response !== null) {
            $this->handleStreamException(new Http2StreamException("Stream headers already received", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $status = (int) $pseudo[":status"];
        if ($status === Status::SWITCHING_PROTOCOLS) {
            $this->handleConnectionException(new Http2ConnectionException("Switching Protocols (101) is not part of HTTP/2", Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $response = new Response('2', $status, Status::getReason($status), $headers, new InMemoryStream(), $stream->request);
        if ($status < 200) {
            ...
            if ($onInformationalResponse !== null) {
                $stream->preResponseResolution = call(function () use($onInformationalResponse, $response, $stream, $streamId) {
                    ...
                    try {
                        ...
                        $this->handleStreamException(new Http2StreamException('Informational response handler threw an exception', $streamId, Http2Parser::CANCEL));
                    }
                });
            }
            return;
        ...
        $cancellationToken = new CombinedCancellationToken($stream->cancellationToken, $bodyCancellation->getToken());
        $response->setBody(new ResponseBodyStream(new IteratorStream($stream->body->iterate()), $bodyCancellation));
        ...
        if (isset($headers["content-length"])) {
            if (\count($headers['content-length']) !== 1) {
                $this->handleStreamException(new Http2StreamException("Multiple content-length header values", $streamId, Http2Parser::PROTOCOL_ERROR));
                return;
            }
            $contentLength = $headers["content-length"][0];
            if (!\preg_match('/^(0|[1-9][0-9]*)$/', $contentLength)) {
                $this->handleStreamException(new Http2StreamException("Invalid content-length header value", $streamId, Http2Parser::PROTOCOL_ERROR));
                return;
            }
            $stream->expectedLength = (int) $contentLength;
        }
        $cancellationToken->subscribe(function (CancelledException $exception) use($streamId) {
            if (!isset($this->streams[$streamId])) {
                return;
            }
            ...
            if (!$this->streams[$streamId]->originalCancellation->isRequested()) {
                ...
                $exception = new TimeoutException('Allowed transfer timeout exceeded, took longer than ' . $transferTimeout . ' ms', 0, $exception);
            }
            $this->releaseStream($streamId, $exception);
        });
        unset($bodyCancellation, $cancellationToken);
        // Remove reference to cancellation token.
    }
    public function handlePushPromise(int $parentId, int $streamId, array $pseudo, array $headers)
    {
        foreach ($pseudo as $name => $value) {
        ...
        if (!isset($pseudo[":method"], $pseudo[":path"], $pseudo[":scheme"], $pseudo[":authority"]) || isset($headers["connection"]) || $pseudo[":path"] === '' || isset($headers["te"]) && \implode($headers["te"]) !== "trailers") {
            $this->handleStreamException(new Http2StreamException("Invalid header values", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        ...
        if ($method !== 'GET' && $method !== 'HEAD') {
            $this->handleStreamException(new Http2StreamException("Pushed request method must be a safe method", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        if (!\preg_match("#^([A-Z\\d.\\-]+|\\[[\\d:]+])(?::([1-9]\\d*))?\$#i", $host, $matches)) {
            $this->handleStreamException(new Http2StreamException("Invalid pushed authority (host) name", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $host = $matches[1];
        $port = isset($matches[2]) ? (int) $matches[2] : $this->socket->getRemoteAddress()->getPort();
        if (!isset($this->streams[$parentId])) {
            $this->handleStreamException(new Http2StreamException("Parent stream {$parentId} is no longer open", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        /** @var Http2Stream $parentStream */
        $parentStream = $this->streams[$parentId];
        if (\strcasecmp($host, $parentStream->request->getUri()->getHost()) !== 0) {
            $this->handleStreamException(new Http2StreamException("Authority does not match original request authority", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        ...
        try {
            $uri = Uri\Http::createFromComponents(["scheme" => $scheme, "host" => $host, "port" => $port, "path" => $target, "query" => $query]);
        ...
        $stream = new Http2Stream($streamId, $request, HttpStream::fromStream($parentStream->stream, static function () {
            throw new \Error('Calling Stream::request() on a pushed request is forbidden');
        }, static function () {
            // nothing to do
        }), $parentStream->cancellationToken, $parentStream->originalCancellation, self::DEFAULT_WINDOW_SIZE, 0);
    ...
    public function handleData(int $streamId, string $data)
    {
        ...
        $stream = $this->streams[$streamId];
        if (!$stream->body) {
            $this->handleStreamException(new Http2StreamException("Stream headers not complete or body already complete", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $length = \strlen($data);
        ...
        if ($stream->expectedLength !== null && $stream->received > $stream->expectedLength) {
            $this->handleStreamException(new Http2StreamException("Body size exceeded content-length in header", $streamId, Http2Parser::CANCEL));
            return;
        }
        $this->increaseConnectionWindow();
        $promise = $stream->body->emit($data);
        $promise->onResolve(function (\Throwable $exception = null) use($streamId) {
    ...
    public function handleStreamEnd(int $streamId)
    {
        ...
        $stream = $this->streams[$streamId];
        if ($stream->expectedLength !== null && $stream->received !== $stream->expectedLength) {
            $this->handleStreamException(new Http2StreamException("Body length does not match content-length header", $streamId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $body = $stream->body;
        $stream->body = null;
    ...
    public function reserveStream()
    {
        --$this->remainingStreams;
    }
    public function unreserveStream()
    {
        ++$this->remainingStreams;
    }
    ...
    public function request(Request $request, CancellationToken $cancellationToken, Stream $stream) : Promise
    {
        return call(function () use($request, $cancellationToken, $stream) : \Generator {
            ...
            if ($this->socket->isClosed()) {
                $exception = new UnprocessedRequestException(new SocketException(\sprintf("Socket to '%s' closed before the request could be sent", $this->socket->getRemoteAddress())));
                foreach ($request->getEventListeners() as $eventListener) {
                    (yield $eventListener->abort($request, $exception));
                }
                throw $exception;
            ...
            if ($request->getTransferTimeout() > 0) {
                $cancellationToken = new CombinedCancellationToken($cancellationToken, new TimeoutCancellationToken($request->getTransferTimeout()));
            }
            $streamId = $this->streamId += 2;
            // Client streams should be odd-numbered, starting at 1.
            $this->streams[$streamId] = $http2stream = new Http2Stream($streamId, $request, $stream, $cancellationToken, $originalCancellation, self::DEFAULT_WINDOW_SIZE, $this->initialWindowSize);
            $this->socket->reference();
            $transferTimeout = $request->getTransferTimeout();
            $cancellationId = $cancellationToken->subscribe(function (CancelledException $exception) use($streamId, $originalCancellation, $transferTimeout) {
                if (!isset($this->streams[$streamId])) {
                    return;
                }
                $this->writeFrame(Http2Parser::RST_STREAM, Http2Parser::NO_FLAG, $streamId, \pack("N", Http2Parser::CANCEL));
    ...
    private function run() : \Generator
    {
        try {
            ...
            (yield $this->writeFrame(Http2Parser::SETTINGS, 0, 0, \pack("nNnNnNnN", Http2Parser::ENABLE_PUSH, 1, Http2Parser::MAX_CONCURRENT_STREAMS, 256, Http2Parser::INITIAL_WINDOW_SIZE, self::DEFAULT_WINDOW_SIZE, Http2Parser::MAX_FRAME_SIZE, self::DEFAULT_MAX_FRAME_SIZE)));
            ...
            $this->shutdown(null, new ClientHttp2ConnectionException("The HTTP/2 connection closed unexpectedly", Http2Parser::INTERNAL_ERROR, $exception));
        }
    }
    ...
    private function applySetting(int $setting, int $value)
    {
        switch ($setting) {
            case Http2Parser::INITIAL_WINDOW_SIZE:
                if ($value > 2147483647) {
                    // (1 << 31) - 1
                    $this->handleConnectionException(new Http2ConnectionException("Invalid window size: {$value}", Http2Parser::FLOW_CONTROL_ERROR));
                    return;
                }
            ...
            case Http2Parser::MAX_FRAME_SIZE:
                if ($value < 1 << 14 || $value >= 1 << 24) {
                    $this->handleConnectionException(new Http2ConnectionException("Invalid maximum frame size: {$value}", Http2Parser::PROTOCOL_ERROR));
                    return;
                }
            ...
            case Http2Parser::MAX_CONCURRENT_STREAMS:
                if ($value > 2147483647) {
                    // (1 << 31) - 1
                    $this->handleConnectionException(new Http2ConnectionException("Invalid concurrent streams value: {$value}", Http2Parser::PROTOCOL_ERROR));
                    return;
                }
            ...
            // TODO Respect this setting from the server
    ...
    private function writeBufferedData(Http2Stream $stream) : Promise
    {
        ...
        if ($windowSize > 0) {
            ...
            $promise = $this->writeFrame(Http2Parser::DATA, Http2Parser::NO_FLAG, $stream->id, \substr($data, $off, $windowSize - $off));
    ...
    private function releaseStream(int $streamId, \Throwable $exception = null)
    {
        \assert(isset($this->streams[$streamId]));
        $stream = $this->streams[$streamId];
        if ($stream->responsePending || $stream->body || $stream->trailers) {
            $exception = $exception ?? new ClientHttp2StreamException(\sprintf("Stream %d closed unexpectedly", $streamId), $streamId, Http2Parser::INTERNAL_ERROR);
    ...
    private function setupPingIfIdle()
    {
    ...
    private function cancelIdleWatcher()
    {
        if ($this->idleWatcher !== null) {
    ...
    private function generateHeaders(Request $request) : array
    {
        ...
        $headers = [[":authority", $authority], [":path", $path], [":scheme", $uri->getScheme()], [":method", $request->getMethod()]];
    ...
    private function increaseConnectionWindow()
    {
        $increase = 0;
    ...
    private function increaseStreamWindow(Http2Stream $stream)
    {
