...
final class ConnectionLimitingPool implements ConnectionPool
{
    ...
    public function getStream(Request $request, CancellationToken $cancellation) : Promise
    {
        return call(function () use($request, $cancellation) {
            ...
            /** @var Stream $stream */
            list($connection, $stream) = (yield from $this->getStreamFor($uri, $request, $cancellation));
            ...
            return HttpStream::fromStream($stream, coroutine(function (Request $request, CancellationToken $cancellationToken) use($connection, $stream, $uri) {
                ...
                $response->getTrailers()->onResolve(function () use($connection, $uri) {
                    $this->onReadyConnection($connection, $uri);
                });
                return $response;
            }), function () use($connection, $uri) {
                $this->onReadyConnection($connection, $uri);
            });
        });
    }
    private function getStreamFor(string $uri, Request $request, CancellationToken $cancellation) : \Generator
    {
        ...
        do {
            ...
            $deferredPromise->onResolve(function () use($uri, $deferredId) {
        ...
        $connectionPromise->onResolve(function (\Throwable $exception = null, Connection $connection = null) use(&$deferred, $uri, $connectionId, $isHttps) {
            if ($exception) {
            ...
            $connection->onClose(function () use($uri, $connectionId) {
                $this->openConnectionCount--;
                $this->dropConnection($uri, $connectionId);
            });
        });
        try {
            ...
            list($exception) = $exception->getReasons();
    ...
    private function onReadyConnection(Connection $connection, string $uri)
    {
        $connectionId = \spl_object_id($connection);
    ...
    private function removeWaiting(string $uri, int $deferredId)
    {
    ...
    private function dropConnection(string $uri, int $connectionId)
    {
