...
final class Http1Connection implements Connection
{
    ...
    use ForbidCloning;
    const MAX_KEEP_ALIVE_TIMEOUT = 60;
    ...
    public function getStream(Request $request) : Promise
    {
        ...
        $this->busy = true;
        return new Success(HttpStream::fromConnection($this, $this->callableFromInstanceMethod('request'), $this->callableFromInstanceMethod('release')));
    }
    ...
    private function readResponse(Request $request, CancellationToken $originalCancellation, CancellationToken $readingCancellation, Stream $stream) : \Generator
    {
        ...
        $trailers = [];
        $trailersCallback = static function (array $headers) use(&$trailers) {
            $trailers = $headers;
        };
        ...
        try {
            while (null !== ($chunk = (yield $this->socket->read()))) {
                ...
                // Read body async
                asyncCall(function () use($parser, $request, $response, $bodyEmitter, $trailersDeferred, $originalCancellation, $readingCancellation, $bodyCancellationToken, $stream, &$backpressure, &$trailers) {
            ...
            $originalCancellation->throwIfRequested();
            throw new SocketException(\sprintf("Receiving the response headers for '%s' failed, because the socket to '%s' @ '%s' closed early with %d bytes received within %d milliseconds", $request->getUri()->withUserInfo(''), $request->getUri()->withUserInfo('')->getAuthority(), $this->socket->getRemoteAddress(), \strlen($parser->getBuffer()), getCurrentTime() - $start));
