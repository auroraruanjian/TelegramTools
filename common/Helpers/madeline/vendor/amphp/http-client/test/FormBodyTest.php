...
class FormBodyTest extends AsyncTestCase
{
    ...
    {
        $body = new FormBody();
        $body->addFields(['a' => 'a'], 'application/json');
        $body->addField('b', 'b', 'application/json');
        $body->addField('c', 'c', '');
        $body->addField('d', 'd');
        $body->addFields(['e' => 'e'], '');
        $body->addFields(['f' => 'f']);
        $this->assertSame([['a', 'a', 'application/json', null], ['b', 'b', 'application/json', null], ['c', 'c', '', null], ['d', 'd', 'text/plain', null], ['e', 'e', '', null], ['f', 'f', 'text/plain', null]], $body->getFields());
