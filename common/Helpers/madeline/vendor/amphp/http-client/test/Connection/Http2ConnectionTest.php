...
class Http2ConnectionTest extends AsyncTestCase
{
    ...
    {
        $hpack = new HPack();
        list($server, $client) = Socket\createPair();
        ...
        $server->write(self::packFrame($hpack->encode([[":status", Status::CONTINUE], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
        $server->write(self::packFrame($hpack->encode([[":status", Status::NO_CONTENT], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS | Http2Parser::END_STREAM, 1));
    ...
    public function testSwitchingProtocols() : \Generator
    {
        ...
        /** @var Stream $stream */
        $stream = (yield $connection->getStream($request));
        $server->write(self::packFrame($hpack->encode([[":status", Status::SWITCHING_PROTOCOLS], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
    ...
    public function testTrailers() : \Generator
    {
        ...
        asyncCall(static function () use($server, $hpack) {
            (yield delay(100));
            $server->write(self::packFrame($hpack->encode([[":status", Status::OK], ["content-length", "4"], ["trailers", "Foo"], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
            (yield delay(100));
            $server->write(self::packFrame('test', Http2Parser::DATA, 0, 1));
            (yield delay(100));
            $server->write(self::packFrame($hpack->encode([["foo", 'bar']]), Http2Parser::HEADERS, Http2Parser::END_HEADERS | Http2Parser::END_STREAM, 1));
        });
        /** @var Response $response */
        $response = (yield $stream->request($request, new NullCancellationToken()));
    ...
    public function testTrailersWithoutTrailers() : \Generator
    {
        ...
        /** @var Stream $stream */
        $stream = (yield $connection->getStream($request));
        $server->write(self::packFrame($hpack->encode([[":status", Status::OK], ["content-length", "4"], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
    ...
    public function testCancellingWhileStreamingBody() : \Generator
    {
        ...
        asyncCall(static function () use($server, $hpack) {
            (yield delay(100));
            $server->write(self::packFrame($hpack->encode([[":status", Status::OK], ["content-length", "8"], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
            (yield delay(100));
    ...
    public function testCancellingPushPromiseBody() : \Generator
    {
        ...
        $request->setPushHandler(function (Request $request, Promise $response) use(&$pushPromise) {
        ...
        asyncCall(static function () use($server, $hpack) {
            (yield delay(100));
            $server->write(self::packFrame($hpack->encode([[":status", Status::OK], ["content-length", "4"], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 1));
            $server->write(self::packFrame(\pack("N", 3) . $hpack->encode([[":method", 'GET'], [":authority", 'localhost'], [":scheme", 'https'], [":path", '/static']]), Http2Parser::PUSH_PROMISE, Http2Parser::END_HEADERS, 1));
            $server->write(self::packFrame($hpack->encode([[":status", Status::OK], ["content-length", "4"], ["date", formatDateHeader()]]), Http2Parser::HEADERS, Http2Parser::END_HEADERS, 3));
            (yield delay(100));
            $server->write(self::packFrame('test', Http2Parser::DATA, Http2Parser::END_STREAM, 1));
            (yield delay(1000));
