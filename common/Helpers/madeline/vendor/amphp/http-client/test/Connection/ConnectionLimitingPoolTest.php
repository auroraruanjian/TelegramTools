...
class ConnectionLimitingPoolTest extends AsyncTestCase
{
    ...
    {
        $client = (new HttpClientBuilder())->usingPool(ConnectionLimitingPool::byAuthority(1))->build();
        $this->setTimeout(5000);
        $this->setMinimumRuntime(2000);
        (yield [$client->request(new Request('http://httpbin.org/delay/1')), $client->request(new Request('http://httpbin.org/delay/1'))]);
    }
    public function testTwoConnections() : \Generator
    {
        $client = (new HttpClientBuilder())->usingPool(ConnectionLimitingPool::byAuthority(2))->build();
    ...
    private function createMockConnection(Request $request) : Connection
    {
        ...
        $stream->method('request')->willReturn(new Delayed(100, $response));
        $stream->method('getLocalAddress')->willReturn(new SocketAddress('127.0.0.1'));
        $stream->method('getRemoteAddress')->willReturn(new SocketAddress('127.0.0.1'));
        $connection = $this->createMock(Connection::class);
        $connection->method('getStream')->willReturn(new Success($stream));
        $connection->method('getProtocolVersions')->willReturn(['1.1', '1.0']);
        return $connection;
    }
    public function testWaitForConnectionToBecomeAvailable() : \Generator
    {
        ...
        $factory->expects($this->exactly(1))->method('create')->willReturn(new Success($connection));
    ...
    public function testConnectionBecomingAvailableWhileConnecting() : \Generator
    {
        ...
        $connection = $this->createMockConnection($request);
        $factory = $this->createMock(ConnectionFactory::class);
        $factory->expects($this->exactly(2))->method('create')->willReturnCallback(function () use($connection) : Promise {
