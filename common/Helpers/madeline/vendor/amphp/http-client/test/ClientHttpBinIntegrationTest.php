...
class ClientHttpBinIntegrationTest extends AsyncTestCase
{
    ...
    public function testHeaderCase() : \Generator
    {
        ...
        $body = (yield $response->getBody()->buffer());
        $result = \json_decode($body, true);
        $this->assertSame([['tEst', 'test'], ['accept', '*/*'], ['user-agent', 'amphp/http-client @ v4.x'], ['Accept-Encoding', 'gzip, deflate, identity'], ['host', (string) $this->socket->getAddress()]], $result);
    }
    ...
    {
        $this->givenSlowRawServerResponse(100, "HTTP/1.1 200 OK\r\nConnection: close\r\nContent-Length: 1000\r\n\r\n",...\array_fill(0, 1000, '.'));
    ...
    public function testHttp2SupportLargeBody() : \Generator
    {
        ...
        // larger than initial stream window
        /** @var Response $response */
        $response = (yield $this->client->request($request));
        $body = (yield $response->getBody()->buffer());
        $json = \json_decode($body, true);
    ...
    private function givenRawServerResponse(string $response)
    {
    ...
    private function givenSlowRawServerResponse(int $delay, ...$chunks)
    {
    ...
    private function givenApplicationInterceptor(ApplicationInterceptor $interceptor)
    {
        $this->builder = $this->builder->intercept($interceptor);
        $this->client = $this->builder->build();
    }
    private function givenNetworkInterceptor(NetworkInterceptor $interceptor)
    {
