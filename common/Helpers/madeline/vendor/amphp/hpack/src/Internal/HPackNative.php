...
final class HPackNative
{
    ...
    private static function huffmanLookupInit() : array
    {
        ...
        $memoize = [];
        for ($off = 7; $off > 0; $off--) {
            foreach ($terminals[$off] as list($key, $next)) {
    ...
    public static function huffmanDecode(string $input)
    {
        ...
        $length = \strlen($input);
        $out = \str_repeat("\0", $length / 5 * 8 + 1);
        // max length
        // Fail if EOS symbol is found.
        if (\strpos($input, "?ÿÿÿ") !== false) {
            return null;
        }
        for ($bitCount = $off = $i = 0; $i < $length; $i++) {
            list($lookup, $chr) = $huffmanLookup[$lookup][$input[$i]];
            if ($chr === null) {
                continue;
            }
            if ($chr === "") {
