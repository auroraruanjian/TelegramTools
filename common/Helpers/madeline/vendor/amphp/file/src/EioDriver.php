...
final class EioDriver implements Driver
{
    ...
    private function onOpenHandle(array $openArr, $result, $req)
    {
    ...
    private function onOpenFtruncate(array $openArr, $result, $req)
    {
    ...
    private function onOpenFstat(array $openArr, $result, $req)
    {
        list($fh, $mode, $path, $deferred) = $openArr;
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
    ...
    private function onStat(array $data, $result, $req)
    {
    ...
    public function size(string $path) : Promise
    {
        ...
        $this->stat($path)->onResolve(function ($error, $result) use($deferred) {
            if (empty($result)) {
                $deferred->fail(new FilesystemException("Specified path does not exist"));
            } elseif ($result["mode"] & \EIO_S_IFREG) {
                $deferred->resolve($result["size"]);
            } else {
                $deferred->fail(new FilesystemException("Specified path is not a regular file"));
            }
        });
        return $deferred->promise();
    }
    ...
    private function onLstat(Deferred $deferred, $result, $req)
    {
        if ($result === -1) {
            $deferred->resolve(null);
        } else {
    ...
    private function onReadlink(Deferred $deferred, $result, $req)
    {
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
        } else {
    ...
    private function onGenericResult(Deferred $deferred, $result, $req)
    {
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
        } else {
    ...
    private function onUnlink(array $data, $result, $req)
    {
        list($deferred, $path) = $data;
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
    ...
    public function mkdir(string $path, int $mode = 0777, bool $recursive = false) : Promise
    {
        ...
        if ($recursive) {
            ...
            $tmpPath = "";
            $callback = function () use(&$callback, &$arrayPath, &$tmpPath, $mode, $priority, $deferred) {
                $tmpPath .= DIRECTORY_SEPARATOR . \array_shift($arrayPath);
                if (empty($arrayPath)) {
                    \eio_mkdir($tmpPath, $mode, $priority, [$this, "onGenericResult"], $deferred);
                } else {
                    $this->isdir($tmpPath)->onResolve(function ($error, $result) use($callback, $tmpPath, $mode, $priority) {
                        if ($result) {
                            $callback();
                        } else {
    ...
    private function onRmdir(array $data, $result, $req)
    {
        list($deferred, $path) = $data;
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
    ...
    private function onScandir(Deferred $deferred, $result, $req)
    {
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
        } else {
    ...
    private function onGetOpen(Deferred $deferred, $result, $req)
    {
        if ($result === -1) {
            $deferred->fail(new FilesystemException(\eio_get_last_error($req)));
        } else {
    ...
    private function onGetFstat(array $fhAndPromisor, $result, $req)
    {
    ...
    private function onGetRead(array $fhAndPromisor, $result, $req)
    {
        list($fh, $deferred) = $fhAndPromisor;
        \eio_close($fh);
        if ($result === -1) {
    ...
    private function onPutOpen(array $data, $result, $req)
    {
    ...
    private function onPutWrite(array $fhAndPromisor, $result, $req)
    {
        list($fh, $deferred) = $fhAndPromisor;
        \eio_close($fh);
        if ($result === -1) {
