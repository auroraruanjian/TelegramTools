...
final class UvDriver implements Driver
{
    ...
    public function open(string $path, string $mode) : Promise
    {
        ...
        \uv_fs_open($this->loop, $path, $flags, $chmod, function ($fh) use($openArr) {
            if ($fh) {
                ...
                $deferred->fail(new FilesystemException("Failed opening file handle to {$path}"));
            }
        });
        ...
        if ($mode[0] === "w") {
            \uv_fs_ftruncate($this->loop, $fh, $length = 0, function ($fh) use($openArr) {
                if ($fh) {
                    ...
                    list(, $path, $deferred) = $openArr;
                    $deferred->fail(new FilesystemException("Failed truncating file {$path}"));
                }
            });
        } else {
            \uv_fs_fstat($this->loop, $fh, function ($fh, $stat) use($openArr) {
                if ($fh) {
                    ...
                    list(, $path, $deferred) = $openArr;
                    $deferred->fail(new FilesystemException("Failed reading file size from open handle pointing to {$path}"));
                }
            });
        }
    }
    ...
    public function mkdir(string $path, int $mode = 0777, bool $recursive = false) : Promise
    {
        ...
        if ($recursive) {
            ...
            $tmpPath = "";
            $callback = function () use(&$callback, &$arrayPath, &$tmpPath, $mode, $deferred) {
                ...
                if (empty($arrayPath)) {
                    ...
                    $this->isdir($tmpPath)->onResolve(function ($error, $result) use($callback, $tmpPath, $mode) {
                        if ($result) {
                            $callback();
                        } else {
