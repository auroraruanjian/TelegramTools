...
final class BlockingDriver implements Driver
{
    ...
    public function open(string $path, string $mode) : Promise
    {
        ...
        if (!($fh = \fopen($path, $mode . 'be'))) {
            return new Failure(new FilesystemException("Failed opening file handle"));
        }
    ...
    {
        if (!@\file_exists($path)) {
            return new Failure(new FilesystemException("Path does not exist"));
        }
        if (!@\is_file($path)) {
            return new Failure(new FilesystemException("Path is not a regular file"));
        }
    ...
    {
        if (!@\is_dir($path)) {
            return new Failure(new FilesystemException("Not a directory"));
        ...
        return new Failure(new FilesystemException("Failed reading contents from {$path}"));
    }
    /**
     * {@inheritdoc}
     */
