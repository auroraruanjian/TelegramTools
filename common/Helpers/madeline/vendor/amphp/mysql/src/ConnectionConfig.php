...
final class ConnectionConfig extends SqlConnectionConfig
{
    ...
    const BIN_CHARSET = 45;
    // utf8mb4_general_ci
    const KEY_MAP = ['hostname' => 'host', 'username' => 'user', 'pass' => 'password', 'database' => 'db', 'dbname' => 'db', 'compress' => 'compression', 'useCompression' => 'compression', 'cs' => 'charset', 'localInfile' => 'local-infile'];
    ...
    public static function fromString(string $connectionString, ConnectContext $context = null) : self
    {
        ...
        return new self($parts['host'], (int) ($parts['port'] ?? self::DEFAULT_PORT), $parts['user'] ?? null, $parts['password'] ?? null, $parts['db'] ?? null, $context, $parts['charset'] ?? self::DEFAULT_CHARSET, $parts['collate'] ?? self::DEFAULT_COLLATE, ($parts['compression'] ?? '') === 'on', ($parts['local-infile'] ?? '') === 'on');
    }
    public function __construct(string $host, int $port = self::DEFAULT_PORT, string $user = null, string $password = null, string $database = null, ConnectContext $context = null, string $charset = self::DEFAULT_CHARSET, string $collate = self::DEFAULT_COLLATE, bool $useCompression = false, string $key = '', bool $useLocalInfile = false)
    {
