...
{
    const SERVER_STATUS_IN_TRANS = 0x1;
    // a transaction is active
    const SERVER_STATUS_AUTOCOMMIT = 0x2;
    // auto-commit is enabled
    const SERVER_MORE_RESULTS_EXISTS = 0x8;
    const SERVER_STATUS_NO_GOOD_INDEX_USED = 0x10;
    const SERVER_STATUS_NO_INDEX_USED = 0x20;
    const SERVER_STATUS_CURSOR_EXISTS = 0x40;
    // Used by Binary Protocol Resultset to signal that COM_STMT_FETCH has to be used to fetch the row-data.
    const SERVER_STATUS_LAST_ROW_SENT = 0x80;
    const SERVER_STATUS_DB_DROPPED = 0x100;
    const SERVER_STATUS_NO_BACKSLASH_ESCAPES = 0x200;
    const SERVER_STATUS_METADATA_CHANGED = 0x400;
    const SERVER_QUERY_WAS_SLOW = 0x800;
...
{
    const SESSION_TRACK_SYSTEM_VARIABLES = 0x0;
    const SESSION_TRACK_SCHEMA = 0x1;
    const SESSION_TRACK_STATE_CHANGE = 0x2;
}
/** @internal */
...
REGEX;
    ...
    const CLIENT_LOCAL_INFILE = 0x80;
    ...
    // $params is by-ref, because the actual result object might not yet have been filled completely with data upon call of this ...
    public function execute(int $stmtId, string $query, array &$params, array $prebound, array $data = []) : Promise
    {
        ...
        return $deferred->promise();
        // do not use $this->startCommand(), that might unexpectedly reset the seqId!
    }
    /** @see 14.7.7 COM_STMT_CLOSE */
    public function closeStmt(int $stmtId)
    {
        $payload = "\31" . DataTypes::encodeInt32($stmtId);
        $this->appendTask(function () use($payload) {
            if ($this->connectionState === self::READY) {
                $this->resetIds();
                $this->sendPacket($payload);
                $this->resetIds();
                // does not expect a reply - must be reset immediately
            }
            $this->ready();
        });
    }
    ...
    public function listFields(string $table, string $like = "%") : Promise
    {
        return $this->startCommand(static function () use($table, $like) {
            $this->sendPacket("\4{$table}\0{$like}");
    ...
    public function refresh(int $subcommand) : Promise
    {
        return $this->startCommand(function () use($subcommand) {
            $this->sendPacket("\7" . \chr($subcommand));
        });
    }
    ...
    public function killProcess(int $process) : Promise
    {
        return $this->startCommand(function () use($process) {
            $this->sendPacket("\f" . DataTypes::encodeInt32($process));
        });
    }
    ...
    {
        $payload = "\32" . DataTypes::encodeInt32($stmtId);
        $deferred = new Deferred();
    ...
    {
        $payload = "\34" . DataTypes::encodeInt32($stmtId) . DataTypes::encodeInt32(1);
        $deferred = new Deferred();
        $this->appendTask(function () use($payload, $deferred) {
            $this->resetIds();
            $this->addDeferred($deferred);
    ...
    private function handleError(string $packet)
    {
        $off = 1;
    ...
    private function parseOk(string $packet)
    {
        $off = 1;
        ...
        if ($this->capabilities & self::CLIENT_SESSION_TRACK) {
            // Even though it seems required according to 14.1.3.1, there is no length encoded string, i.e. no trailing NULL ....???
    ...
    private function handleOk(string $packet)
    {
        $this->parseOk($packet);
    ...
    private function parseEof(string $packet)
    {
        if ($this->capabilities & self::CLIENT_PROTOCOL_41) {
    ...
    private function handleEof(string $packet)
    {
    ...
    private function handleHandshake(string $packet)
    {
        $off = 1;
        $this->protocol = \ord($packet);
        if ($this->protocol !== 0xa) {
    ...
    private function handleLocalInfileRequest(string $packet)
    {
    ...
    private function handleQuery(string $packet)
    {
        switch (\ord($packet)) {
            case self::OK_PACKET:
                $this->parseOk($packet);
    ...
    private function handleExecute(string $packet)
    {
    ...
    private function handleFieldList(string $packet)
    {
    ...
    private function handleTextColumnDefinition(string $packet)
    {
        $this->handleColumnDefinition($packet, "handleTextResultSetRow");
    }
    private function handleBinaryColumnDefinition(string $packet)
    {
        $this->handleColumnDefinition($packet, "handleBinaryResultSetRow");
    }
    private function handleColumnDefinition(string $packet, string $cbMethod)
    {
    ...
    private function prepareParams(string $packet)
    {
        if (!$this->result->columnsToFetch--) {
    ...
    private function prepareFields(string $packet)
    {
        if (!$this->result->columnsToFetch--) {
            $this->parseCallback = null;
            $this->query = null;
    ...
    private function handleTextResultSetRow(string $packet)
    {
    ...
    private function handleBinaryResultSetRow(string $packet)
    {
        $packettype = \ord($packet);
        if ($packettype === self::EOF_PACKET) {
            $this->parseEof($packet);
    ...
    private function handlePrepare(string $packet)
    {
        switch (\ord($packet)) {
            case self::OK_PACKET:
                break;
    ...
    private function readStatistics(string $packet)
    {
    ...
    private function compilePacket(string $pending) : string
    {
        ...
        do {
            ...
            $packet .= \substr_replace(\pack("V", $len), \chr(++$this->seqId), 3, 1) . $out;
            // expects $len < (1 << 24) - 1
        } while ($pending !== "");
        return $packet;
    }
    ...
    private function parsePayload(string $packet)
    {
    ...
    private function authSwitchRequest(string $packet)
    {
        $this->parseCallback = null;
        switch (\ord($packet)) {
            case self::EOF_PACKET:
                ...
                $len = \strpos($packet, "\0");
                $pluginName = \substr($packet, 0, $len);
                // @TODO mysql_native_pass only now...
     ...
     */
    private function sendHandshake(bool $inSSL = false)
    {
