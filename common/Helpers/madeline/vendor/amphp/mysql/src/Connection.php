...
final class Connection implements Link
{
     ...
     * @return Promise<self>
     */
    public static function connect(ConnectionConfig $config, CancellationToken $token = null, Socket\Connector $connector = null) : Promise
    {
        $token = $token ?? new NullCancellationToken();
        return call(function () use($config, $token, $connector) {
            $socket = (yield ($connector ?? Socket\connector())->connect($config->getConnectionString(), $config->getConnectContext(), $token));
