...
class AllTest extends BaseTest
{
    ...
    public function testPendingPromiseArray()
    {
        Loop::run(function () use(&$result) {
            $promises = [new Delayed(20, 1), new Delayed(30, 2), new Delayed(10, 3)];
            $callback = function ($exception, $value) use(&$result) {
                $result = $value;
            };
            Promise\all($promises)->onResolve($callback);
    ...
    {
        Loop::run(function () use(&$result) {
            $promises = [new Delayed(20, 1), new FulfilledPromise(2)];
            $callback = function ($exception, $value) use(&$result) {
                $result = $value;
            };
            Promise\all($promises)->onResolve($callback);
    ...
    public function testArrayKeysPreserved()
    {
        ...
        Loop::run(function () use(&$result) {
            $promises = ['one' => new Delayed(20, 1), 'two' => new Delayed(30, 2), 'three' => new Delayed(10, 3)];
            $callback = function ($exception, $value) use(&$result) {
                $result = $value;
            };
            Promise\all($promises)->onResolve($callback);
