...
abstract class DriverTest extends TestCase
{
     ...
     * @return callable
     */
    public abstract function getFactory() : callable;
    /** @var Driver */
    public $loop;
    ...
    public function testNoMemoryLeak($type, $args)
    {
        ...
        $this->start(function (Driver $loop) use($type, $args, $runs) {
            ...
            $cb($runs);
            /* just to set up eventual structures inside loop without counting towards memory comparison */
            \gc_collect_cycles();
            ...
            $this->assertLessThan($runs * 4, $endMem - $initialMem);
            // * 4, as 4 is minimal sizeof(void *)
        });
    }
    /**
                ...
                $loop->defer(function () use($loop, $def2, $del5, $f) {
                    ...
                    $loop->defer(function () use($loop, $del5, $f) {
                        ...
                        $loop->defer(function () use($loop, $f) {
                            \usleep(700000);
                            // to have $msDelay == 500 and $msDelay == 600 run at the same tick (but not $msDelay == 150)
                            $loop->defer(function () use($loop, $f) {
                                $loop->defer($f(4, 0));
                            });
                        });
    ...
    public function testDeferEnabledInNextTick()
    {
        ...
        $tick();
        // disable + immediate enable after a tick should have no effect either
    ...
    {
        return [["string"], [0], [PHP_INT_MIN], [-1.0], [true], [false], [[]], [null], [new \StdClass()]];
    }
    public function testRethrowsFromCallbacks()
    {
        foreach (["onReadable", "onWritable", "defer", "delay", "repeat", "onSignal"] as $watcher) {
            $promises = [new Failure(new \Exception("rethrow test")), new RejectedReactPromise(new \Exception("rethrow test")), new Coroutine((function () {
                if (false) {
                    yield;
                }
    ...
    {
        \usleep(600000);
        // 600ms instead of 500ms to allow for variations in timing.
    ...
    public function testBug163ConsecutiveDelayed()
    {
        ...
        $this->loop->run();
        $this->assertGreaterThan(100 * $emits - 1, $time * 1000);
    }
}