...
class Parsedown
{
    ...
    protected function blockCode($Line, $Block = null)
    {
        ...
        if ($Line['indent'] >= 4) {
            ...
            $Block = array('element' => array('name' => 'pre', 'element' => array('name' => 'code', 'text' => $text)));
            return $Block;
        }
    }
    ...
    protected function blockFencedCode($Line)
    {
        ...
        $Block = array('char' => $marker, 'openerLength' => $openerLength, 'element' => array('name' => 'pre', 'element' => $Element));
        return $Block;
    }
    ...
    protected function blockHeader($Line)
    {
        ...
        $text = trim($text, ' ');
        $Block = array('element' => array('name' => 'h' . min(6, $level), 'handler' => array('function' => 'lineElements', 'argument' => $text, 'destination' => 'elements')));
        return $Block;
    }
    #
    # List
    ...
    protected function blockQuote($Line)
    {
        if (preg_match('/^>[ ]?+(.*+)/', $Line['text'], $matches)) {
            $Block = array('element' => array('name' => 'blockquote', 'handler' => array('function' => 'linesElements', 'argument' => (array) $matches[1], 'destination' => 'elements')));
            return $Block;
        }
    }
    ...
    protected function blockMarkup($Line)
    {
        ...
        if (preg_match('/^<[\\/]?+(\\w*)(?:[ ]*+' . $this->regexHtmlAttribute . ')*+[ ]*+(\\/)?>/', $Line['text'], $matches)) {
            ...
            $Block = array('name' => $matches[1], 'element' => array('rawHtml' => $Line['text'], 'autobreak' => true));
            return $Block;
        }
    }
    ...
    protected function blockReference($Line)
    {
        if (strpos($Line['text'], ']') !== false and preg_match('/^\\[(.+?)\\]:[ ]*+<?(\\S+?)>?(?:[ ]+["\'(](.+)["\')])?[ ]*+$/', $Line['text'], $matches)) {
            ...
            $Data = array('url' => $matches[2], 'title' => isset($matches[3]) ? $matches[3] : null);
            $this->DefinitionData['Reference'][$id] = $Data;
            $Block = array('element' => array());
            return $Block;
        }
    }
    #
    ...
    protected function blockTableContinue($Line, array $Block)
    {
        ...
        if (count($Block['alignments']) === 1 or $Line['text'][0] === '|' or strpos($Line['text'], '|')) {
            ...
            $Element = array('name' => 'tr', 'elements' => $Elements);
    ...
    # Inline Elements
    #
    protected $InlineTypes = array('!' => array('Image'), '&' => array('SpecialCharacter'), '*' => array('Emphasis'), ':' => array('Url'), '<' => array('UrlTag', 'EmailTag', 'Markup'), '[' => array('Link'), '_' => array('Emphasis'), '`' => array('Code'), '~' => array('Strikethrough'), '\\' => array('EscapeSequence'));
    # ~
