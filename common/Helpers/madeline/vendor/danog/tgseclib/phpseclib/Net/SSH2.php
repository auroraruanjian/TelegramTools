 ...
 *    include 'vendor/autoload.php';
 *
 *    $key = \tgseclib\Crypt\PublicKeyLoader::load('...', '(optional) password');
 *
 *    $ssh = new \tgseclib\Net\SSH2('www.domain.tld');
 ...
 */
namespace tgseclib\Net;

use tgseclib\Crypt\Blowfish;
...
use tgseclib\Crypt\Twofish;
use tgseclib\Crypt\ChaCha20;
use tgseclib\Math\BigInteger;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use tgseclib\System\SSH\Agent;
use tgseclib\System\SSH\Agent\Identity as AgentIdentity;
use tgseclib\Exception\NoSupportedAlgorithmsException;
use tgseclib\Exception\UnsupportedAlgorithmException;
use tgseclib\Exception\UnsupportedCurveException;
use tgseclib\Exception\ConnectionClosedException;
...
class SSH2
{
     ...
     *     the other side.
     *
     * @see \tgseclib\Net\SSH2::send_channel_packet()
     * @see \tgseclib\Net\SSH2::get_channel_packet()
     * @access private
     */
     ...
     * @throws \UnexpectedValueException on receipt of unexpected packets
     * @throws \RuntimeException on other errors
     * @throws \tgseclib\Exception\NoSupportedAlgorithmsException when none of the algorithms phpseclib has loaded are compatible
     * @access private
     */
     ...
     * \tgseclib\Crypt\Common\SymmetricKey.
     *
     * @param string $algorithm Name of the encryption algorithm
     * @return mixed Instance of \tgseclib\Crypt\Common\SymmetricKey or null for unknown
     * @access private
     */
     ...
     *
     * @param string $algorithm Name of the encryption algorithm
     * @return mixed Instance of \tgseclib\Crypt\Hash or null for unknown
     * @access private
     */
     ...
     * Login
     *
     * The $password parameter can be a plaintext password, a \tgseclib\Crypt\RSA object or an array
     *
    ...
    private function login_helper($username, $password = null)
    {
        ...
        if (!is_string($password)) {
            throw new \UnexpectedValueException('$password needs to either be an instance of \\tgseclib\\Crypt\\Common\\PrivateKey, \\System\\SSH\\Agent, an array or a string');
        }
     ...
     *
     * @param string $username
     * @param \tgseclib\System\SSH\Agent $agent
     * @return bool
     * @access private
     */
     ...
     *
     * @param string $username
     * @param \tgseclib\Crypt\Common\PrivateKey $privatekey
     * @return bool
     * @throws \RuntimeException on connection error
     * @access private
     * @internal It might be worthwhile, at some point, to protect against {@link http://tools.ietf.org/html/rfc4251#section-9.3.9 traffic analysis}
            ...
            if ($publickey instanceof EC) {
                ...
                switch ($curveName) {
                    ...
                    default:
                        ...
                        throw new UnsupportedCurveException('Named Curve of ' . $curveName . ' is not supported by tgseclib\'s SSH2 implementation');
                }
            } else {
                if ($publickey instanceof DSA) {
                    $privatekey = $privatekey->withSignatureFormat('SSH2');
                    $hash = 'sha1';
     ...
     * Execute Command
     *
     * If $callback is set to false then \tgseclib\Net\SSH2::get_channel_packet(self::CHANNEL_EXEC) will need to be called manually.
    ...
    public function exec($command, $callback = null)
    {
        ...
        if ($this->request_pty === true) {
            ...
            $packet = Strings::packSSH2('CNsCsN4s', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_EXEC], 'pty-req', 1, 'vt100', $this->windowColumns, $this->windowRows, 0, 0, $terminal_modes);
            $this->send_binary_packet($packet);
            $response = $this->get_binary_packet();
            if ($response === false) {
                $this->bitmap = 0;
        ...
        // down.  the one place where it might be desirable is if you're doing something like \tgseclib\Net\SSH2::exec('ping localhost &').
    ...
    private function initShell()
    {
        ...
        $terminal_modes = pack('C', NET_SSH2_TTY_OP_END);
        $packet = Strings::packSSH2('CNsCsN4s', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SHELL], 'pty-req', 1, 'vt100', $this->windowColumns, $this->windowRows, 0, 0, $terminal_modes);
        $this->send_binary_packet($packet);
        $response = $this->get_binary_packet();
        if ($response === false) {
            $this->bitmap = 0;
    ...
    private function get_binary_packet($skip_channel_filter = false)
    {
        ...
        if ($this->decrypt) {
            switch ($this->decrypt->name) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    $this->decrypt->setNonce($this->decrypt->fixed . $this->decrypt->invocation_counter);
        ...
        if ($this->hmac_check instanceof Hash) {
            ...
            $reconstructed = !$this->hmac_check->etm ? pack('NCa*', $packet_length, $padding_length, $payload . $padding) : $encrypted;
    ...
    protected function send_binary_packet($data, $logged = null)
    {
        ...
        if ($this->hmac_create instanceof Hash && !$this->hmac_create->etm) {
            if (($this->hmac_create->getHash() & "ÿÿÿÿ") == 'umac') {
        ...
        if ($this->encrypt) {
            switch ($this->encrypt->name) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    $this->encrypt->setNonce($this->encrypt->fixed . $this->encrypt->invocation_counter);
                    Strings::increment_str($this->encrypt->invocation_counter);
                    $this->encrypt->setAAD($temp = $packet & "ÿÿÿÿ");
                ...
                case 'chacha20-poly1305@openssh.com':
                    ...
                    $length = $this->lengthEncrypt->encrypt($packet & "ÿÿÿÿ");
                    $this->encrypt->setCounter(0);
                    // this is the same approach that's implemented in Salsa20::createPoly1305Key()
                    // but we don't want to use the same AEAD construction that RFC8439 describes
                    // for ChaCha20-Poly1305 so we won't rely on it (see Salsa20::poly1305())
                    $this->encrypt->setPoly1305Key($this->encrypt->encrypt(str_repeat("\0", 32)));
                    $this->encrypt->setAAD($length);
                    $this->encrypt->setCounter(1);
                    $packet = $length . $this->encrypt->encrypt(substr($packet, 4));
                    break;
                default:
                    $packet = $this->hmac_create instanceof Hash && $this->hmac_create->etm ? ($packet & "ÿÿÿÿ") . $this->encrypt->encrypt(substr($packet, 4)) : $this->encrypt->encrypt($packet);
            }
        }
        if ($this->hmac_create instanceof Hash && $this->hmac_create->etm) {
            if (($this->hmac_create->getHash() & "ÿÿÿÿ") == 'umac') {
     ...
     *
     * \tgseclib\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally closed by the server
    ...
    {
        $this->connect();
        return ['kex' => $this->kex_algorithm, 'hostkey' => $this->signature_format, 'client_to_server' => ['crypt' => $this->encrypt->name, 'mac' => $this->hmac_create->name, 'comp' => 'none'], 'server_to_client' => ['crypt' => $this->decrypt->name, 'mac' => $this->hmac_check->name, 'comp' => 'none']];
    }
    /**
     ...
     * @throws \tgseclib\Exception\NoSupportedAlgorithmsException when the key isn't in a supported format
     * @access public
     */
