...
use tgseclib\Exceptions\NoKeyLoadedException;
...
class PrivateKey extends RSA implements Common\PrivateKey
{
     ...
     *
     * @access private
     * @param \tgseclib\Math\BigInteger $c
     ...
     *
     * As a consequence of this, a private key encrypted ciphertext produced with \tgseclib\Crypt\RSA may not decrypt
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                $options += ['hash' => $this->hash->getHash(), 'MGFHash' => $this->mgfHash->getHash(), 'saltLength' => $this->sLen];
            } else {
