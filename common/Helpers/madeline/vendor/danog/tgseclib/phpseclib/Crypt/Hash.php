...
class Hash
{
    ...
    private function computeKey()
    {
        ...
        $this->computedKey = is_array($this->hash) ? call_user_func($this->hash, $this->key) : hash($this->hash, $this->key, true);
    }
    /**
     ...
     * @param BigInteger $k integer in the range ... prime(wordbits) - 1.
     * @param string $m string with length divisible by (wordbits / 8) bytes.
     * @return integer in the range ... prime(wordbits) - 1.
     */
    ...
    public function hash($text)
    {
        if ($this->hash == 'umac') {
            if ($this->recomputeAESKey) {
                ...
                if (!isset(self::$maxwordrange64)) {
                    ...
                    $prime36 = new BigInteger("\0\0\0\17ÿÿÿû", 256);
                    self::$factory36 = new PrimeField($prime36);
                    $prime64 = new BigInteger("ÿÿÿÿÿÿÿÅ", 256);
                    self::$factory64 = new PrimeField($prime64);
                    $prime128 = new BigInteger("ÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿa", 256);
        ...
        $output = !empty($this->key) || is_string($this->key) ? hash_hmac($this->hash, $text, $this->computedKey, true) : hash($this->hash, $text, true);
