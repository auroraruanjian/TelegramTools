...
class X509
{
    ...
    {
        if (!is_string($extnId)) {
            // eg. if it's a \tgseclib\File\ASN1\Element object
            return true;
        }
        switch ($extnId) {
            case 'id-ce-keyUsage':
    ...
    public function sign($issuer, $subject)
    {
        ...
        if ($this->caFlag) {
            ...
            $this->setExtension('id-ce-basicConstraints', array_unique(array_merge(['cA' => true], $basicConstraints)), true);
            if (!isset($subject->currentKeyIdentifier)) {
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            ...
            case $key instanceof X509:
                ...
                return false;
            default:
                // Should be a key object (i.e.: \tgseclib\Crypt\RSA).
