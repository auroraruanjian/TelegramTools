...
final class Rfc8484StubResolver implements Resolver
{
    ...
    public function resolve(string $name, int $typeRestriction = null) : Promise
    {
        ...
        return call(function () use($name, $typeRestriction) {
            ...
            if ($this->dohConfig->isNameserver($name)) {
                // Work around an OPCache issue that returns an empty array with "return ...",
                ...
                try {
                    if ($typeRestriction) {
                        ...
                        try {
                            ...
                            break;
                            // Break redirect loop, otherwise we query the same records 5 times
    ...
    public function query(string $name, int $type) : Promise
    {
        ...
        $promise = call(function () use($name, $type) {
            ...
            $timeout = new TimeoutException(\sprintf("No response for '%s' (%s) from any nameserver after %d attempts, tried %s", $name, Record::getName($type), $attempts, \implode(", ", $attemptDescription)));
            if (!$exceptions) {
                throw $timeout;
            }
