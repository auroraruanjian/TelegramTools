...
class Stream
{
    ...
    public function streamOpen($path, $mode, $options, &$opened_path)
    {
        ...
        if (!\is_array($opt[self::WRAPPER_NAME]) || !isset($opt[self::WRAPPER_NAME]['handler']) || !$opt[self::WRAPPER_NAME]['handler'] instanceof Handler || !isset($opt[self::WRAPPER_NAME]['stream_id'])) {
            return false;
        }
