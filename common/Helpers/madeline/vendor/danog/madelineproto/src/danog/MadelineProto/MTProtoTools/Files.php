...
trait Files
{
    ...
    public function uploadFromTgfile($media, $cb = null, bool $encrypted = false) : \Generator
    {
        ...
        $bridge = new class($size, $chunk_size, $cb)
        {
             ...
             */
            public function __construct(int $size, int $partSize, callable $cb = null)
            {
             ...
             *
             * @param ...$params Params to be passed to cb
             *
             * @return void
             */
     ...
     * @return ?array
     */
    public static function extractBotAPIFile(array $info)
    {
    ...
    public function downloadToBrowser($messageMedia, callable $cb = null) : \Generator
    {
        ...
        $result = self::parseHeaders($_SERVER['REQUEST_METHOD'], $headers, $messageMedia);
    ...
    public function downloadToResponse($messageMedia, ServerRequest $request, callable $cb = null) : \Generator
    {
        ...
        $messageMedia = (yield from $this->getDownloadInfo($messageMedia));
        $result = self::parseHeaders($request->getMethod(), \array_map(function (array $headers) {
            return $headers[0];
        }, $request->getHeaders()), $messageMedia);
        $body = null;
        if ($result['serve']) {
            $body = new IteratorStream(new Producer(function (callable $emit) use(&$messageMedia, &$cb, &$result) {
    ...
    private static function parseHeaders(string $method, array $headers, array $messageMedia) : array
    {
        if (isset($headers['range'])) {
            ...
            if ($size_unit == 'bytes') {
                ...
                return ['serve' => false, 'code' => Status::RANGE_NOT_SATISFIABLE, 'headers' => self::NO_CACHE];
            }
        } else {
            $range = '';
        }
        ...
        $result = ['serve' => $method !== 'HEAD', 'code' => Status::OK, 'headers' => []];
