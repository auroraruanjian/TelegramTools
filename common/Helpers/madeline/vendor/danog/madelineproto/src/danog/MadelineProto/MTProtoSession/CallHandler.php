...
trait CallHandler
{
    ...
    public function methodCallAsyncRead(string $method, $args = [], array $aargs = ['msg_id' => null]) : \Generator
    {
        ...
        if (\is_array($readDeferred)) {
            $readDeferred = Tools::all(\array_map(function (Deferred $value) {
    ...
    public function methodCallAsyncWrite(string $method, $args = [], array $aargs = ['msg_id' => null]) : \Generator
    {
        ...
        $message = \array_merge($aargs, ['_' => $method, 'type' => $methodInfo['type'], 'contentRelated' => $this->contentRelated($method), 'promise' => $deferred, 'method' => true, 'unencrypted' => !$this->shared->hasTempAuthKey() && \strpos($method, '.') === false]);
