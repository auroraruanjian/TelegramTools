...
class MyEventHandler extends \danog\MadelineProto\EventHandler
{
     ...
     * @param ?API $API API
     */
    public function __construct(APIWrapper $API = null)
    {
    ...
    public function onUpdateNewMessage(array $update) : \Generator
    {
        ...
        try {
            ...
            $url = new \danog\MadelineProto\FileCallback($url, function ($progress, $speed, $time) use($peerId, $id) {
            ...
            (yield $this->messages->sendMedia(['peer' => $peerId, 'reply_to_msg_id' => $messageId, 'media' => ['_' => 'inputMediaUploadedDocument', 'file' => $url, 'attributes' => [['_' => 'documentAttributeFilename', 'file_name' => $name]]], 'message' => 'Powered by @MadelineProto!', 'parse_mode' => 'Markdown']));
...
$settings = ['logger' => ['logger_level' => 4], 'serialization' => ['serialization_interval' => 30], 'connection_settings' => ['media_socket_count' => ['min' => 20, 'max' => 1000]], 'upload' => ['allow_automatic_upload' => false]];
