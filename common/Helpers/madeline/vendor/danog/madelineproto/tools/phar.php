...
function ___install_madeline()
{
    ...
    if ($custom_branch === '') {
        // If the constant is an empty string, default to the latest alpha build
        $custom_branch = 'master';
    } elseif ($custom_branch === null) {
        // If the constant is not defined, default to the latest stable build
        $custom_branch = '';
    }
    ...
    if (!\file_exists('madeline.phar') || !\file_exists('madeline.phar.version') || \file_get_contents('madeline.phar.version') !== $release) {
        ...
        if ($phar) {
            ...
            foreach ($current as $name => $version) {
                ...
                $postData['downloads'][] = ['name' => $name, 'version' => $version];
            }
            $opts = ['http' => ['method' => 'POST', 'header' => ['Content-Type: application/json'], 'content' => \json_encode($postData), 'timeout' => 6]];
