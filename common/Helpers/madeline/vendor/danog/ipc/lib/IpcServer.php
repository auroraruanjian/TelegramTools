...
class IpcServer
{
    ...
    public function __construct(string $uri = '', bool $useFIFO = false)
    {
        ...
        $this->watcher = Loop::onReadable($this->server, static function (string $watcher, $server) use(&$acceptor, $fifo) {
            if ($fifo) {
                $length = \unpack('v', \fread($server, 2))[1];
                if (!$length) {
                    return;
                    // Could not accept, wrong length read
                }
                $prefix = \fread($server, $length);
                $sockets = [$prefix . '1', $prefix . '2'];
     ...
     * @see Loop::reference()
     */
    public final function reference()
    {
     ...
     */
    public final function unreference()
    {
