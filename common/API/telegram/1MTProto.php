 ...
 */
class MTProto extends \danog\MadelineProto\MTProto
{
    ...
    */
    ...
    const NO_CACHE = ['Cache-Control' => ['no-store, no-cache, must-revalidate, max-age=0', 'post-check=0, pre-check=0'], 'Pragma' => 'no-cache'];
    /**
    ...
    public static function parseSettings(array $settings, array $previousSettings = []) : array
    {
        ...
        $default_settings = ['authorization' => [
            ...
            // Update callback
            'run_callback' => true,
        ], 'secret_chats' => ['accept_chats' => true], 'serialization' => ['serialization_interval' => 30, 'cleanup_before_serialization' => false], 'upload' => ['allow_automatic_upload' => true, 'part_size' => 512 * 1024, 'parallel_chunks' => 20], 'download' => ['report_broken_media' => true, 'part_size' => 1024 * 1024, 'parallel_chunks' => 20], 'pwr' => [
            'pwr' => false,
            // Need info ?
            'db_token' => false,
            // Need info ?
    ...
    public function report(string $message) : \Generator
    {
        ...
        if ($this->settings['logger']['logger'] === Logger::FILE_LOGGER && ($path = $this->settings['logger']['logger_param'])) {
            ...
            if (!(yield exists($path))) {
                ...
                $file = (yield from $this->methodCallAsyncRead('messages.uploadMedia', ['peer' => $this->reportDest[0], 'media' => ['_' => 'inputMediaUploadedDocument', 'file' => $path, 'attributes' => [['_' => 'documentAttributeFilename', 'file_name' => 'MadelineProto.log']]]]));
            }
        }
        $sent = true;
